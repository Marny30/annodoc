...
// parse .ann format with multiline, return brat document data format
    var parseAnn2 = function(ann, log) {
        if (log === undefined) {
            log = makeLogger();
        }
        ...
        var document_text = tmp[0];
	var lines_to_parse = tmp[1].split('\n');
        // parse lines after the first as .ann-formatted lines
        // (see e.g. http://brat.nlplab.org/standoff.html)
        var textbounds = [],
            relations = [],
            ...
            comments = [],
            error = false;
        for (var i=0; i<lines_to_parse.length; i++) {
            var line = lines_to_parse[i];
...
//                    '</span></span>';
    ...
    var parseFunctionMap = {
 	...
 	// .ann standoff wit multiline 
        '.ann-annotation-multiline' : parseAnn2,
        '.language-ann-multiline' : parseAnn2,
    };
